...
The sidebar uses Pi's supported non-capturing overlay seam and leaves Pi's renderer untouched. It starts at 44 columns, can be resized between 28 and 72 columns, and auto-hides below 92 terminal columns. Because the stable overlay is drawn at the right edge instead of reflowing Pi's workspace, content beneath those columns may be covered while the sidebar is shown.
...
Pi 0.84 cannot switch between regular and fullscreen renderers while any overlay is open. Hide the sidebar with `/atelier sidebar off`, switch TUI mode, then show it again with `/atelier sidebar on`.
